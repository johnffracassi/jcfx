...
select * from iv_bat_inns where batsman = 'jeff';
...
group by match_id);
select * from dv_top_team_score order by date asc;

// displayable batsman scores
...
select * from dv_top_batsman_score;

select score, ps_runs
from iv_batting_detailed a, iv_matches b
where batsman = 'jeff' and a.match_id = b.match_id
order by b.date asc;
...
select *, (cum_score/inns) cum_avg, (cum_score/cum_bf*100) cum_sr
from iv_cum_batting 
where batsman = 'mick';
...
group by batsman with rollup);

select batsman, matches, inns, runs, bf, (runs/inns) avg, (runs/bf*100) sr, ps_runs, (runs/ps_runs) contrib, 
    ex, (ex*2/runs) ex_perc_runs, (ex/bf) ex_perc_bf, (bwld/w) bwl, (cgt/w) cgt, (ro/w) ro, (st/w) st,
    (r9*9/runs) r9, (r8*8/runs) r8, (r7*7/runs) r7, (r6*6/runs) r6, (r5*5/runs) r5, (r4*4/runs) r4, 
    (r3*3/runs) r3, (r2*2/runs) r2, (r1*1/runs) r1
...
select batsman, inns, runs, bf, 
    ...
    ((r9+r8+r7+r6+r5)/bf*100) back_net_perc, ((r4+r3+r2+r1)/bf*100) other_perc, (r0/bf*100) dots_perc
from iv_batting_detailed_agg 
where inns >=5 order by dots_perc asc;
order by back_net desc, other desc, dots asc;

select * from iv_batting_detailed_agg;
select * from iv_partnership_to_match;
...
select * from dv_inns_histo where batsman='jeff';

// ball-by-ball histogram
select ex, w, (r7+r8+r9) r7, (r5+r6) r6, r4, r3, r2, r1, r0
from iv_batting_detailed_agg
where batsman = 'jeff';
...
select * from dv_match_summary order by no_score desc limit 10;
select * from dv_match_summary order by dots desc limit 10;
select * from dv_match_summary order by sevens desc limit 10;
select * from dv_match_summary order by wick desc limit 10;
...
where a.batsman = b.batsman and a.season_id = b.season_id);
